...
pub type ParamsView<'a, A, D = Ix2> = ParamsBase<ViewRepr<&'a A>, D>;
/// A type alias for a mutable view of the parameters
pub type ParamsViewMut<'a, A, D = Ix2> = ParamsBase<ViewRepr<&'a mut A>, D>;
/// A type alias for a [`ParamsBase`] with a _borrowed_ internal layout
pub type CowParams<'a, A, D = Ix2> = ParamsBase<CowRepr<'a, A>, D>;
/// A type alias for the [`ParamsBase`] whose elements are of type `*const A` using a
/// [`RawViewRepr`] layout
pub type RawViewParams<A, D = Ix2> = ParamsBase<RawViewRepr<*const A>, D>;
/// A type alias for the [`ParamsBase`] whose elements are of type `*mut A` using a
/// [`RawViewRepr`] layout
pub type RawMutParams<A, D = Ix2> = ParamsBase<RawViewRepr<*mut A>, D>;
