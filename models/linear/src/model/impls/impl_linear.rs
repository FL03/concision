...
impl<T, D> Borrow<Config> for Linear<T, D> where D: RemoveAxis {
    fn borrow(&self) -> &Config {
        &self.config
    }
}

impl<T, D> Borrow<LinearParams<T, D>> for Linear<T, D> where D: RemoveAxis {
    fn borrow(&self) -> &LinearParams<T, D> {
        &self.params
    }
}

impl<T, D> BorrowMut<LinearParams<T, D>> for Linear<T, D> where D: RemoveAxis {
