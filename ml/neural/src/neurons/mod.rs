...
#[cfg(test)]
mod tests {
    use super::activate::{Activator, Softmax};
    use super::*;
    ...
    fn test_neuron() {
        ...
        let a = Neuron::new(Softmax::rho, bias.clone(), a_weights.clone());

        let exp = _artificial(&a_data, Some(bias.clone()), Softmax, &a_weights);
        assert_eq!(a.compute(&a_data), exp);

        let b_data = array![0.0, 9.0, 3.0, 5.0, 3.0];
        let b_weights = array![2.0, 8.0, 8.0, 0.0, 3.0];

        let b = Neuron::new(Softmax::rho, bias.clone(), b_weights.clone());

        let exp = _artificial(&b_data, Some(bias), Softmax, &b_weights);
